  ...
  loginHandler: async (req, res) => {
    ...
    userModel.findByEmail(email, async (err, users) => {
      ...
      if (users.length === 0) {
        return res
          .status(401)
          .json({ message: "This account is not registered." });
      }
      ...
      req.session.role = user.email;
      req.session.role = user.role;

      console.log("Session data after login:", req.session); // Log session data here to confirm it's set

      console.log("Sending response for successful login:");
